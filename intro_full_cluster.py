...
def get_limit_rows(connection:pymysql.Connection,limit:int,offset:int,current_id:int,id:int, gte_date:str, id_desc:bool, counter:str):
    ...
    # Base SQL query
    if id:
        base_sql = f"SELECT c.id, CONCAT(c.introtext, ' ', c.fulltext) AS text, c.alias, c.images, c.title, c.catid FROM `{db_prefix}content` AS c"
    elif current_id != 0:
        offset=0
        base_sql = f"""
            SELECT c.id, CONCAT(c.introtext, ' ', c.fulltext) AS text, c.alias, c.images, c.title, c.catid as total FROM `{db_prefix}content` AS c
            LEFT JOIN `{db_prefix}categories` AS cat ON c.catid = cat.id
            WHERE c.`access` = 1
            AND cat.published = 1
            AND c.catid NOT IN (87, 89, 91, 98, 99, 100, 172, 197, 198, 199, 200, 202, 203, 217, 219)
...
def process_context(contexts:list,logger:Logger, h1_title:str, metadesc:str, title:str, temperature=0):
    ...
    if len(contexts) ==1:
        ...
        async def process_texts_async(contexts, logger):
            async with AsyncOpenAI(api_key=config.get("chatgpt", "OPENAI_SECRET_KEY")) as client:
